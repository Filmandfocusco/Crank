...
if (data.ARRI && data.ARRI['ALEXA Mini LF']) {
  ...
  if (!miniLF.some(m => m.sensor_mode === "3.8K 16:9")) {
    miniLF.push({
...
if (data.RED && !data.RED['RED Monstro 8K VV']) {
  data.RED['RED Monstro 8K VV'] = [
    ...
    { "sensor_mode": "2K FF", "resolution": "2.1K (2048 x 1150)", "max_fps_by_codec": { "REDCODE RAW": 300 } }
  ];
}

// 3. Add Phantom
if (!data.Phantom || !data.Phantom['Phantom Flex 4K']) {
  data.Phantom = data.Phantom || {};
  data.Phantom['Phantom Flex 4K'] = [
